...
- Property checks such as `z.property(...)` and `z.properties(...)` use the checked property path.
...
Custom messages from `z.refine` (and other places where the schema author supplies a `message`) are surfaced verbatim, since they are intentional schema text rather than user input. If your refinement messages embed the validated value (e.g., `` `${input} is not allowed` ``), that value will appear in the formatted error, so keep that in mind for fields that may carry sensitive data.
